...
export function App() {
  ...
  const scriptsQuery = useQuery({ queryKey: ["scripts"], queryFn: fetchScripts });
  ...
  const filteredScripts = useMemo(() => {
    ...
    return scripts.filter((script) => script.script_name.toLowerCase().includes(normalized));
  }, [filter, scripts]);

  const activeScript = detailQuery.data ?? scripts.find((script) => script.script_name === selectedScript) ?? scripts[0];
  ...
  return (
    <main className="min-h-screen bg-panel text-ink">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8">
        ...
        {scriptsQuery.isLoading ? <StateMessage text="Loading script health..." /> : null}
        {scriptsQuery.isError ? <StateMessage text="Unable to load script health." tone="bad" /> : null}
        ...
        <section className="grid gap-6 lg:grid-cols-[minmax(320px,420px)_1fr]">
          ...
          <div>
            {activeScript ? (
              <ScriptDetail script={activeScript} loading={detailQuery.isFetching && selectedScript !== null} />
            ) : (
              <StateMessage text="Post completed sessions to start tracking script health." />
            )}
          </div>
        </section>
      </div>
...
function StateMessage({ text, tone = "neutral" }: { text: string; tone?: "neutral" | "bad" }) {
  return (
    <div className={`rounded-md border px-4 py-3 text-sm ${tone === "bad" ? "border-red-200 bg-red-50 text-bad" : "border-line bg-white text-slate-600"}`}>
      {text}
    </div>
  );
}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="truncate text-base font-semibold">{script.script_name}</div>
          <div className="mt-1 text-xs text-slate-500">Latest {formatDateTime(script.latest_stopped_at)}</div>
        </div>
...
function ScriptDetail({ script, loading }: { script: ScriptHealth; loading: boolean }) {
  return (
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              {script.run_count} runs, {formatRuntime(script.average_runtime_seconds)} average runtime.
            </p>
          </div>
          <div className="flex flex-wrap gap-2 text-xs">
            <Pill icon={<CheckCircle2 className="h-4 w-4" />} label={`${script.recent_success_count} success`} tone="good" />
            <Pill icon={<XCircle className="h-4 w-4" />} label={`${script.recent_failure_count} failure`} tone="bad" />
            <Pill icon={<AlertTriangle className="h-4 w-4" />} label={`${script.recent_unknown_count} unknown`} tone="warn" />
          </div>
        </div>
      </div>
...
function Pill({ icon, label, tone }: { icon: React.ReactNode; label: string; tone: "good" | "bad" | "warn" }) {
  const toneClass = {
  ...
  return <span className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 ${toneClass}`}>{icon}{label}</span>;
}
